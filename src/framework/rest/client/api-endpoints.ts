export const API_ENDPOINTS = {
  PRODUCTS: '/api/product',
  PRODUCTS_POPULAR: '/popular-products',
  PRODUCTS_REVIEWS: '/reviews',
  PRODUCTS_REVIEWS_ABUSE_REPORT: '/abusive_reports',
  PRODUCTS_QUESTIONS: '/questions',
  FEEDBACK: '/feedbacks',
  CATEGORIES: '/api/product-category',
  TAGS: '/tags',
  SHOPS: '/shops',
  AUTHORS: '/authors',
  AUTHORS_TOP: '/top-authors',
  MANUFACTURERS: '/manufacturers',
  MANUFACTURERS_TOP: '/top-manufacturers',
  COUPONS: '/coupons',
  COUPONS_VERIFY: '/coupons/verify',
  ORDERS: '/api/order',
  ORDERS_STATUS_BUYER: '/api/order/status-buyer',
  ORDERS_REFUNDS: '/refunds',
  ORDERS_PAYMENT: '/orders/payment',
  ORDERS_CHECKOUT_VERIFY: '/orders/checkout/verify',
  ORDERS_CHECK: '/api/order/check',
  ORDERS_CHECK_AUCTION: '/api/order/check-auction',
  SHIPPINGS_CALCULATE: '/api/shipping/calculate',
  PAYMENT_METHODS: '/api/payment-method',
  ORDERS_DOWNLOADS: '/downloads',
  GENERATE_DOWNLOADABLE_PRODUCT_LINK: '/downloads/digital_file',
  USERS: '/users',
  USERS_ADDRESS: '/api/user-address',
  USERS_ME: '/api/account',
  USERS_NOTIFICATION: '/api/account/notification',
  USERS_AUCTION: '/api/account/my-auction',
  USERS_LOGIN: '/oauth/token',
  USERS_PARTNER_REGISTER: '/api/partner',
  USERS_REGISTER: '/api/register',
  USERS_REGISTER_VERIFY: '/api/register/verify',
  USERS_FORGOT_PASSWORD: '/api/forgot-password',
  USERS_VERIFY_FORGOT_PASSWORD_TOKEN: '/verify-forget-password-token',
  USERS_RESET_PASSWORD: '/api/reset-password',
  USERS_CHANGE_PASSWORD: '/api/account/update-password',
  USERS_LOGOUT: '/api/account/logout',
  USERS_SUBSCRIBE_TO_NEWSLETTER: '/subscribe-to-newsletter',
  USERS_CONTACT_US: '/contact-us',
  USERS_WISHLIST: '/my-wishlists',
  USERS_SELECT_PROVINCE: '/api/select/provinces',
  USERS_SELECT_CITY: '/api/select/cities',
  USERS_SELECT_DISTRICT: '/api/select/districts',
  WISHLIST: '/api/wishlist',
  CART: '/api/cart',
  CART_UPDATE: '/api/cart/bulk',
  USERS_WISHLIST_TOGGLE: '/api/wishlist',
  SOCIAL_LOGIN: '/social-login-token',
  SEND_OTP_CODE: '/send-otp-code',
  VERIFY_OTP_CODE: '/verify-otp-code',
  OTP_LOGIN: '/otp-login',
  UPDATE_CONTACT: '/update-contact',
  UPLOADS: '/attachments',
  MY_QUESTIONS: '/my-questions',
  MY_REPORTS: '/my-reports',
  CARDS: '/cards',
  SET_DEFAULT_CARD: '/set-default-card',
  SAVE_PAYMENT_METHOD: '/save-payment-method',
  PAYMENT_INTENT: '/payment-intent',
};

...
import { useNewProduct } from '@/framework/product';
...
export default function NewProductGrid({
  className,
  limit = 10,
  variables,
}: Props) {
  const { t } = useTranslation('common');
  const { products, isLoading, error } = useNewProduct(variables);
  ...
  return (
    <SectionBlock title={t('text-new-arrival')} href="search">

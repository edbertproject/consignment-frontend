...
import { useIncomingProduct } from '@/framework/product';
...
export default function IncomingProducts({
  ...
  const { products, isLoading, error } = useIncomingProduct(variables);

  if (error) return <ErrorMessage message={error.message} />;
  if (!isLoading && !products.length) {
    return (
      <SectionBlock title={t('text-incoming-products')}>
  ...
  return (
    <SectionBlock title={t('text-incoming-products')} href="search">

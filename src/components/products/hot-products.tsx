...
import { useHotProducts } from '@/framework/product';
...
export default function HotProductGrid({
  className,
  limit = 10,
  variables,
}: Props) {
  const { t } = useTranslation('common');
  const { products, isLoading, error } = useHotProducts(variables);

  if (error) return <ErrorMessage message={error.message} />;
  if (!isLoading && !products.length) {
    return (
      <SectionBlock title={t('text-hot-products')}>
